...
export default defineConfig({
  ...
  test: {
    ...
    environment: 'jsdom',
    include: ['src/link/*.test.{ts,tsx}'],
    exclude: ['**/node_modules/**', '**/dist/**'],
    coverage: {
      include: ['src/link/*.{ts,tsx}'],
  ...
  build: {
    ...
    lib: {
      entry: 'src/index.ts',
      name: 'Nextjs-ArcoDesign-Link',
      formats: ['cjs', 'es', 'umd'],
      fileName: (format) => `index.${format === 'es' ? 'esm' : format === 'cjs' ? 'cmd' : format}.js`
    },
    rollupOptions: {
      // 确保外部化处理那些你不想打包进库的依赖
      external: ['react', 'react-dom', 'react/jsx-runtime', 'next/link', '@arco-design/web-react', 'classnames'],
      output: {
        // 为各种格式提供全局变量名
        globals: {
          react: 'react',
